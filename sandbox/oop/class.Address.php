<?php 		
	...
	abstract class Address {
		...
		function __construct($data = array()) {
			...
			if (!is_array($data)) {
				trigger_error("Unable to construct address with a " . get_calss($name));
			}
		...
		function __get($name) {
			...
			// Attempt to reurn a protected property by name.
		...
		/**
		 * Guess the postal code given the city and state.
		...
		protected function _postal_code_guess(){
			...
			$state_name = $mysqli->real_escape_string($this->state_name);
			$sql_query .= ' AND state_name = "' . $state_name . '" ';
		...
		function display() {
			...
			if ($this->street_address_2) {
				$output .= "<br />" . $this->street_address_2 . "<br />";
			}
			$output .= $this->city_name . ", " . $this->state_name . " " . $this->postal_code;
